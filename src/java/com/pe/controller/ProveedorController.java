...
public class ProveedorController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accion.equals("buscarPorIdGRM")) {
            this.BuscarPorIdGRM(request, response);
        }
    }
    ...
    private void BuscarPorIdReq(HttpServletRequest request, HttpServletResponse response)
        ...
        int codigo = Integer.parseInt(request.getParameter("idClienteOC"));
        AuxiliarDAO pdao = new AuxiliarDAO();

        Auxiliar obj = pdao.BuscarPorId(codigo);

        request.getSession().setAttribute("clienteReq", obj);

        response.sendRedirect("InsertarRequerimiento.jsp");

    }

    private void BuscarPorIdGRM(HttpServletRequest request, HttpServletResponse response)
            throws IOException, IOException {

        int codigo = Integer.parseInt(request.getParameter("idProveedorGRM"));
        AuxiliarDAO pdao = new AuxiliarDAO();

        Auxiliar obj = pdao.BuscarPorId(codigo);

        request.getSession().setAttribute("proveedorGRM", obj);
