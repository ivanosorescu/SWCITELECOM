 ...
 * @author yenny
 */
public class RefNotadeSalidaController extends HttpServlet {
    ...
    DecimalFormat formateadorrefns;

    public RefNotadeSalidaController() {
        nsDAO = new MovimientoDAO();
        formateadorrefns = new DecimalFormat("00000000");
    }

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String accion = request.getParameter("accion");
        if (accion.equals("RegistrarrefNotadesalida")) {
            this.RegistrarrefNotadesalida(request, response);
        }
    ...
    private void procesarCarritorefni(HttpServletRequest request, HttpServletResponse response)
        ...
        response.sendRedirect("RefInsertarNotadeSalida.jsp");
    ...
    private void anadirProductorefni(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("RefInsertarNotadeSalida.jsp").forward(
                request, response);
    }

    protected void refNotaIActualizarcantidad(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String acceso = "RefInsertarNotadeSalida.jsp";
    ...
    private void RegistrarrefNotadesalida(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession sesion = request.getSession();
        ...
        v.setSerie("NS01");
        int Nnipt = nsDAO.BuscarNnotasalidamp();
        Nnipt = Nnipt + 1;
        String format = formateadorrefns.format(Nnipt);
        ...
        if (nsDAO.insertarMov(v, detalle)) {
